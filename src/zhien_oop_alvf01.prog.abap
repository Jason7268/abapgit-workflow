...
CLASS lcl_alv_report DEFINITION.
  ...
  PUBLIC SECTION.
    METHODS:
      getso_data,
*        IMPORTING
*          s_vbeln    TYPE ty_r_vbeln
*        CHANGING
*          lt_saledoc TYPE ty_t_saledoc,
      get_address,
*              IMPORTING
*         lt_saledoc    TYPE ty_t_saledoc
*        CHANGING
...
CLASS lcl_alv_report IMPLEMENTATION.
  ...
  METHOD getso_data .
    ...
    FROM  vbak AS v INNER JOIN kna1 AS k
      ON  v~kunnr = k~kunnr
    INTO TABLE lt_saledoc UP TO 20 ROWS
    WHERE v~vbeln IN  s_vbeln.
  ENDMETHOD.

  METHOD get_address .
    CHECK lt_saledoc  IS NOT INITIAL.
    SELECT  addrnumber
            name1
            city1
      FROM adrc INTO TABLE lt_addresscus
      FOR ALL ENTRIES IN lt_saledoc
      WHERE addrnumber = lt_saledoc-adrnr.
  ENDMETHOD.

  METHOD  move_data_output .
*  DATA ls_outputalv  TYPE gt_outputalv.
    SORT lt_saledoc BY vbeln.
    LOOP AT  lt_saledoc INTO DATA(ls_saledoc).
*    ls_outputalv-VBELN      = ls_saledoc-VBELN.
*    ls_outputalv-KUNNR      = ls_saledoc-KUNNR.
*    ls_outputalv-ADDRNUMBER = ls_saledoc-ADRNR.
      READ TABLE lt_addresscus  INTO DATA(ls_addresscus) WITH KEY addrnumber =  ls_saledoc-adrnr.
      IF sy-subrc = 0.
        ls_saledoc-name1 =  ls_addresscus-name1.
        ls_saledoc-city1 =  ls_addresscus-city1.
      ENDIF.
      MODIFY lt_saledoc FROM ls_saledoc .
    ENDLOOP.
  ENDMETHOD.
...
*  ENDMETHOD.
  ...
  METHOD Handle_Ucomm .
*  READ TABLE lt_saledoc INTO DATA(ls_saledoc) INDEX is_seltext-tabindex.
*  IF sy-subrc = 0.
*
